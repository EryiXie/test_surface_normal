...
parser = argparse.ArgumentParser(description='Normal Net Testing Training Script')
# Basic Settings
parser.add_argument('--dataset', default=None, type=str,
                    help='If specified, override the dataset specified in the config with this one.')
parser.add_argument('--config', default='NormalNet_base_config',
...
parser.add_argument('--save_interval', default=5700, type=int,
...
class NetLoss(nn.Module):
    ...
    def __init__(self, net:TestNet, criterion:Loss):
        super().__init__()
        self.net = net
...
class CustomDataParallel(nn.DataParallel):
    ...
    def prepare_data(self, datum, devices:list=None, allocation:list=None):
        ...
        batched_images, batched_gt_normals = datum
        cur_idx = 0
        for device, alloc in zip(devices, allocation):
            for _ in range(alloc):
                batched_images[cur_idx]  = gradinator(batched_images[cur_idx].to(device))
                batched_gt_normals[cur_idx]  = gradinator(batched_gt_normals[cur_idx].to(device))
                cur_idx += 1

        cur_idx = 0
        split_images, split_normals = [[None for alloc in allocation] for _ in range(2)]
        for device_idx, alloc in enumerate(allocation):
            split_images[device_idx]    = batched_images[cur_idx:cur_idx+alloc].cuda()
            split_normals[device_idx]    = batched_gt_normals[cur_idx:cur_idx+alloc].cuda()
            cur_idx += alloc
        return split_images, split_normals
...
def train():
    ...
    if args.resume is not None:
        ...
        norm_net.load_weights(args.resume)
    ...
    if not cfg.freeze_bn: norm_net.freeze_bn() # Freeze bn so we don't kill our means
    norm_net(torch.zeros(1, 3, cfg.max_size, cfg.max_size).cuda())
    if not cfg.freeze_bn: norm_net.freeze_bn(True)
    ...
    try:
        for epoch in range(num_epochs):
            ...
            for datum in data_loader:
                ...
                if iteration % 2000 == 0 and iteration != 0:
                    tensorborad_visual_log(epoch, iteration, norm_net, val_dataset, writer)
                ...
                if iteration % args.save_interval == 0 and iteration != args.start_iter:
                    ...
                    norm_net.save_weights(save_path(epoch, iteration))
        ...
        if args.interrupt:
            ...
            norm_net.save_weights(save_path(epoch, repr(iteration) + '_interrupt'))
        exit()
...
def tensorborad_visual_log(epoch, iteration, norm_net, val_dataset, writer: SummaryWriter):
    with torch.no_grad():
        norm_net.eval()
        eval_script.tensorborad_visual_log(epoch, iteration, norm_net, val_dataset, writer)
        norm_net.train()
